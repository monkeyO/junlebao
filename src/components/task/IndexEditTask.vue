...
<div class='hide'>
          ...
          <div class='taskStep'>
            ...
            <div class="taskStep_content" style="padding-top: 10px;">
              ...
              <el-row style="margin-top: 35px;" v-if="activeName != 'second' && activeName != 'fourth'">
                <el-col :span="2.5">
                  执行任务天数：
                </el-col>
          ...
          <div class='taskStep'>
            ...
            <div class="taskStep_content_third">
              <div class="remind_addservice hide" style='font-size:13px'>
                <p>
              ...
              <div class="otherservice">
                      ...
                      <template v-for="(item,index) of taskbxArr">
                        <el-col :span="5.5" :key="item.id" style="margin-right:30px;">
                          <el-radio v-model="taskbxTime.state" :label="item.id" @click.native="addServerListRadio(taskbxTime,'任务保险',taskbxPriceArr[index])">
                            {{item.value}} {{taskbxPriceArr[index]}}金币
                          </el-radio>
                        </el-col>
                      </template>
                    </el-col>
                ...
                <el-row class="row_height" :class="editState?'hide':''">
                ...
                <template v-if="shopType == 'albb'||shopType == 'tb'||shopType == 'tm'">
                  ...
                  <el-row style="height: 170px; border-bottom: 0;"  :class="editState?'hide':''">
                    <el-col :span="2">
                      <span class="color-red hide">*</span>
                      <span>竞品狙击</span>
                    </el-col>
              ...
              <!-- 好评优化 -->
              <div class="otherservice font_size" :class="editState?'hide':''">
                <div id="jump9"></div>
                <p>
                  <span class="color-red hide">*</span>
              ...
              <!-- 好评优化 -->
              <div class="otherservice" :class="editState?'hide':''">
                <!-- 关键词好评 -->
            ...
            <div class="total_submit" style="margin:20px;padding-bottom: 30px;">
              <el-button type="danger" @click.native="submit()" id="submit">提交任务信息</el-button>
            </div>
          </div>
          <!-- 快递发货 -->
          <div class='taskStep hide' style="font-size:16px;" v-if="kuaidiflag">
...
export default {
  ...
  async mounted () {
    ...
    // this.editDatarea = this.$route.query.datarea
    sessionStorage.token = this.$route.query.token
    this.editSelid = this.$route.query.selid
    let orderno = this.$route.query.id
    let datearea = this.$route.query.datarea
    // 如果不是编辑就显示免责弹窗窗
  ...
  methods:　{
    ...
    async getinfor(orderno,datearea){
      let data = await this.ajax({
        url: `PublishInterfaceapi/getorder?tk=${sessionStorage.token}`,
        data: {orderno,datearea,'selid':this.editSelid}
      })
      ...
      this.editState = data1.state==2?true:false
      // 选择平台数据
    ...
    async getBaseData(shopType){
      let data = await this.ajax({
        url: `PublishInterfaceapi/gettaskbase?tk=${sessionStorage.token}`,
        data: {'class':shopType,'selid':this.editSelid},
        needAlert:false
      })
      if(!data) return false
    ...
    async getLabel (id,type){
      if(type == 1){
        ...
        // 获取对应小类数据
        let data = await this.ajax({
          url: `PublishInterfaceapi/label?tk=${sessionStorage.token}`,
          data: {'type':2,'id':id},
          needAlert:false
        })
        if(!data) return false
    ...
    async getPlanData(pctype,ordertype,limitcashtype,price){
      // 获取对应人气计划数据
      let data = await this.ajax({
        url: `PublishInterfaceapi/rewardrank?tk=${sessionStorage.token}`,
    ...
    async getArea (pid,item,item2) {
      ...
      let data = await this.ajax({
        url: `PublishInterfaceapi/addr_of_express?pid=${pid}&t=${sessionStorage.token}`
      })
      if(!data) return false
      if(pid == 0 && item==''){
    ...
    async getAddress(id){
      if(id!=0){
        let data = await this.ajax({
          url: `PublishInterfaceapi/getshopdetail?tk=${sessionStorage.token}`,
          needAlert:false,
          data: {"shopid": id}
        })
        if(!data) return false
    ...
    async checkurl () {
      ...
      if(flag){
        let data = await this.ajax({
          url: `PublishInterfaceapi/geturldetails?tk=${sessionStorage.token}`,
    ...
    async uploadimg (item,inputId,imgId,flag=true) {
      ...
      previewImg (inputId,imgId,flag).then(res=>{
        (async ()=>{
          ...
          // 上传图片数据
          let data = await this.ajax({
            url: `PublishInterfaceapi/uploadpicture?tk=${sessionStorage.token}`,
            data: formData,
    ...
    addServerListRadio (item,name,price){
      ...
      this.$nextTick(()=>{
        ...
        // 图文好评需要*组数 用2代表
        let ordernum = 1
        if(name=="加赏佣金" || name=="好评优化" || name=="关键词好评" ||name=="任务保险" || name=="提升买号权重" || name=="花呗号下单"){ordernum = 0}
    ...
    async saveAddress () {
      ...
      this.addressState = 1
      let data = await this.ajax({
        url: `PublishInterfaceapi/editShop?tk=${sessionStorage.token}`,
    ...
    async submit () {
      ...
      let data1 = {id,datearea,pctype,shopid,clientype,commodityurl,commodityname,goodsdetail,commoditymasterimgurl,commoditypcimgurl,commodityprice,personbuycount,
                  ...
                  limitcredit,limitpoints,limitlabel,havingpic,isfirst,platformlog,logweight,log,logmoney,limitcashtype,ordertotal,ordertotalperson,'selid':this.editSelid}
      ...
      // 获取数据
      let data = await this.ajax({
        url: `PublishInterfaceapi/putorder?tk=${sessionStorage.token}`,
  ...
  computed: {
    ...
    serverSum () {
      ...
      if(this.isvip&&this.setAmount>=10&&this.backtype=='2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb')){
        // 增值服务+平台待返款
        return (sum+ptkuan).toFixed(2)
      }else{
        // 增值服务+平台待返款+平台代确认截图
        return (sum+ptkuan+this.setAmount*parseFloat(this.ptjietu)).toFixed(2)
      }
    },
    // 快递合计
    kuaidiSum () {
