<template>
  <div class='task'>
        ...
        <el-col :span="24">
          ...
          <div class='taskStep'>
            ...
            <div class="taskStep_content taskStep_content_first">
              ...
              <el-row>
                ...
                <el-col :span="3" @click.native="chooseitem('backtype','1')" :class="backtype=='1'?'active_choose':''" v-if="shopType=='tb'||shopType=='tm'||shopType=='albb'">
                  <span class="pctype pctypelf" :class="backtype=='1'?'active':''"></span>
                  <span>本金立返</span>
                  <div class='f'><span>仅需8元起</span></div>
                </el-col>
                <el-col :span="3" :offset="shopType=='jd'||shopType=='pdd'?0:1" @click.native="chooseitem('backtype','2')" :class="backtype=='2'?'active_choose':''">
                  <span class="pctype pctypehf" :class="backtype=='2'?'active':''"></span>
                  <span>本金货返</span>
                  <div class='f' v-if="shopType!='jd' && shopType!='pdd'"><span>比立返每单多2元</span></div>
                </el-col>
                <div id="jump2"></div>
              </el-row>
              ...
              <el-row>
                ...
                <el-col :span="9" class="input_margin_top">
                  <el-input v-model.trim="itemurl" placeholder="请输入内容" @change="checkurl" id="itemurl"></el-input>
                </el-col>
              </el-row>
              ...
              <el-row>
                ...
                <el-col :span="9" class="input_margin_top">
                  <el-input v-model.trim="itemtitle" placeholder="请输入内容"></el-input>
                </el-col>
              </el-row>
              <!-- 主图 -->
              ...
              <el-row>
                ...
                <el-col :span="3" class="input_margin_top">
                  <el-input-number v-model="price" :min="0" :precision="2" :step="1" @change="changePrice"></el-input-number>
                </el-col>
              ...
              <el-row class="row_height">
                ...
                <el-col :span="20" class="p_explain input_margin_top">
                  <p>
                    ...
                    <span class='reject radio-span'>无需买手联系客服。商家每单额外支付<span style="color: #fd5856; font-weight: bold;">{{ptyunfei}}元</span>作为运费押金，任务完成后运费押金将全部退还给商家</span>
                  </p>
              ...
              <el-row style="height: 135px; border-bottom: 0;">
                ...
                <el-col :span="15">
                  <el-input type="textarea" :rows="6" placeholder="可备注买手下单规格/颜色等事宜，该备注不作为强制买手必须完成的任务" v-model.trim="orderRemark"></el-input>
                </el-col>
              </el-row>
            ...
            <div class="taskStep_content" style="padding-top: 10px;">
              ...
              <el-tabs v-model="activeName" type="card"  @tab-click="changeTabs">
                <el-tab-pane label="关键字" name="first" dataid="1">
                  <div class="remind">
                    <p>买家通过商家提供的关键词，使用手机{{shopType|platformFilter}}自然搜索入店完成下单</p>
                    <p>此任务类型推荐急需提升销量或补单的商家选择，适合全品类</p>
                    <p style="color:red;font-size:14px" v-if="(shopType=='tb'||shopType=='tm')&& isvip==false">货返任务：单笔“下单”任务发布50单及以上，每单佣金减1元（除人气任务及回访任务）</p>
                  </div>
                ...
                <el-tab-pane label="关键字(凌晨单)" name="second" v-if="shopType=='tb'||shopType=='tm'" dataid="2">
                  ...
                  <div class="set_keyword_table">
                    ...
                    <!-- 凌晨单新增文案 货返、店铺类型天猫淘宝阿里巴巴才有提示-->
                    <el-row style="margin:20px 0 10px 0;" v-if="(shopType=='tm'||shopType=='tb'||shopType=='albb')&&backtype=='2'">
                      <el-col :span="24">
                        <span class="color-red" v-if="!isvip">
                          VIP商家福利：单品当日发布任务≥10单起，每单立返1-3元（审核截图费用全免）
                          <router-link to="recharge?active=recharge-getvip" style="background: #fd5856;color: white;padding: 4px 10px;font-size: 14px;border-radius: 5px;">
                            升级VIP会员
                          </router-link>
                        </span>
                        <span class="color-red" v-else>
                          专属福利：单品当日发布任务≥10单 每单立返1元；发布任务≥30单  每单立返2元；发布任务≥50单 每单立返3元；（凡是单品当日≥10单 审核截图费用全免）
                        </span>
                      </el-col>
                    </el-row>
                    <!-- 凌晨单特有数据 -->
                ...
                <el-tab-pane label="关键字(人气)" name="third"  dataid="3">
                  <div class="remind">
                    <p>买家通过商家提供的关键词，使用手机{{shopType|platformFilter}}自然搜索入店，完成浏览、收藏、加购、领取优惠券，通过指定的渠道完成下单</p>
                  ...
                  <div class="set_keyword_table">
                    <el-row class="set_keyword_th">
                      <el-col :span="15" style="text-align: start;">
                        <span class="color-red">请设置关键词下的人气,每组关键词人气至少选择1个, 可单选也可以全选,按照需要勾选</span>
                      </el-col>
                      <el-col :span="3">
                        设置关键字：<span class="color-orange">{{keywordArr.length}}</span>组
                      </el-col>
                    ...
                    <el-row class="set_keyword_table_content" v-for="(i, index) in keywordArr" :key="index">
                      <el-row>
                        ...
                        <el-col :span="4">
                          <el-input-number v-model="i.num" :min="0" label="下单数量" @change="calcYongJin"></el-input-number>
                        </el-col>
                        <el-col :span="5" v-if="activeName=='third'">
                          <el-button type="warning" @click="i.setKeywordDialog = true">设置</el-button>
                          <KeyDialog :data="{obj:i,index,getDiscount,getKorder}" @dialog="dialogData" v-if="i.setKeywordDialog"></KeyDialog>
                        </el-col>
                        <el-col :span="5">
                      ...
                      <!-- 关键字人气任务特有数据 -->
                      <el-row class="taskrenqi">
                        <el-col :span="1">
                          <el-checkbox v-model="renqiListAll[index]" @change="chooseRenqiList(index,i.num)">全部</el-checkbox>
                        </el-col>
                        <el-col :span="23">
                            <el-col :span="4.5" v-for="(item,index1) of renqiPlanArr" :key="index1" v-if="activeName=='third'">
                              <el-checkbox v-model="renqiList[index][index1]" :true-label="item.id" @change="chooseRenqi(index,index1,i.num)">{{item.title}}({{item.money}}/单)</el-checkbox>
                            </el-col>
                        </el-col>
                      </el-row>
                    </el-row>
                ...
                <el-tab-pane label="回访" name="fourth" v-if="(shopType=='tb'||shopType=='tm'||shopType=='jd')&&backtype=='2'"  dataid="4">
                  <div class="remind">
                    <template v-if="shopType =='jd'">
                      <p>买手首日通过关键词货比三家后进店，浏览商品+关注商品+关注店铺；</p>
                      <p>次日通过足迹收藏商品或关注的店铺进入商品，浏览评价页+随机浏览副商品+加入购车后下单</p>
                    </template>
                    <template v-else>
                      <p>买手首日通过关键词货比三家后进店，浏览商品+收藏商品+关注店铺；</p>
                      <p>次日通过足迹收藏商品或关注的店铺进入商品，浏览评价页+随机浏览副商品+加入购车后下单</p>
                      <p style="color:red;font-size:14px" v-if="isvip==false">货返任务：单笔“下单”任务发布50单及以上，每单佣金减1元（除人气任务及回访任务）</p>
                    </template>
                    <!-- 接口返回的回访需要的价格表赋值给yongjinArr,此处循环显示 -->
                    <table class="backtable">
                      <tr>
                        <td>
                          类型
                        </td>
                        <td v-for="(item,index) of yongjinArr" :key="index">
                         {{item.title}}
                        </td>
                      </tr>
                      <tr>
                  ...
                  <div class="set_keyword_table">
                    ...
                    <el-row class="set_keyword_table_content" v-for="(i, index) in keywordArr" :key="index">
                      ...
                      <el-col :span="5" v-if="activeName=='fourth'">
                        <el-button type="warning" @click="i.setKeywordDialog = true">设置</el-button>
                        <KeyDialog :data="{obj:i,index,getDiscount,getKorder}" @dialog="dialogData" v-if="i.setKeywordDialog"></KeyDialog>
                      </el-col>
                      <el-col :span="5">
                ...
                <el-tab-pane label="京东快车" name="fifth-x" v-if="shopType=='jd'"  dataid="10">
                  <div class="remind" style="padding:35px;">
                    <p>买家通过商家提供的关键词，通过手机{{shopType|platformFilter}}搜索直通车入店，完成浏览、收藏、加购、领取优惠券等操作后下单</p>
                    <p style="color:red;font-size:14px" v-if="(shopType=='tb'||shopType=='tm')&& isvip==false">货返任务：单笔“下单”任务发布50单及以上，每单佣金减1元（除人气任务及回访任务）</p>
                  </div>
                  ...
                  <div class="taskStep_content_first">
                    <el-row>
                      ...
                      <el-col :span="12" class="input_margin_top">
                        <el-input v-model.trim="tkltkl" placeholder="" ></el-input>
                      </el-col>
                    </el-row>
                ...
                <el-tab-pane label="阿里口令" name="sixth-y" v-if="shopType=='albb'"  dataid="12">
                  ...
                  <div class="taskStep_content_first">
                    <el-row>
                      ...
                      <el-col :span="12" class="input_margin_top">
                        <el-input v-model.trim="tkltkl" placeholder="请输入淘口令" ></el-input>
                      </el-col>
                    </el-row>
                    <el-row style="border-bottom: 0; margin-bottom: 0; padding-bottom: 5px;">
                      <el-col :span="3" style="width: 12%;">
                ...
                <el-tab-pane label="二维码" name="seventh" v-if="shopType=='tb'||shopType=='tm'"  dataid="7">
                  <div class="remind">
                    <p>买家通过商家提供的二维码图片，从{{shopType|platformFilter}}通过扫一扫二维码找到商品，之后完成下单等操作</p>
              ...
              <!-- 所有模式新增文案 货返、店铺类型天猫淘宝阿里巴巴才有提示，凌晨单在上面 -->
              <el-row style="margin:20px 0 10px 0;" v-if="activeName != 'second'&&(shopType=='tm'||shopType=='tb'||shopType=='albb')&&backtype=='2'">
                <el-col :span="24">
                  <span class="color-red" v-if="!isvip">
                    VIP商家福利：单品当日发布任务≥10单起，每单立返1-3元（审核截图费用全免）
                    <router-link to="recharge?active=recharge-getvip" style="background: #fd5856;color: white;padding: 4px 10px;font-size: 14px;border-radius: 5px;">
              ...
              <el-row style="margin-top: 25px;" v-if="activeName != 'second' && activeName != 'fourth'">
                <el-col :span="2.5">
                  执行任务天数：
                </el-col>
                <el-col :span="4" class="input_margin_top">
                  <el-input-number v-model="taskactionday" :min="1" :max="7" :step="1" size="small"></el-input-number>
                </el-col>
          ...
          <div class='taskStep'>
            ...
            <div class="taskStep_content_third">
              <div class="remind_addservice" style='font-size:13px'>
                <p>
                  平台代返款：买手完成后,平台将直接使用押金为您操作返款,商家无需耗费时间、人力处理退款,减少押款周期本次平台代返款费用：
                  <span class='color-red' v-if="price*count<300" >{{setAmount+`单`}}x{{ptfankuan}}金币 = {{setAmount*parseFloat(ptfankuan)|tofixed}}</span>
                  <span class='color-red' v-else >{{price}}元 x {{setAmount}}单 × {{count}}件 x {{ptfankuan}}金币 = {{parseFloat(price)*setAmount*count*parseFloat(ptfankuan)|tofixed}}</span>
                  <!-- <span class='color-red'>{{setAmount+`单`}}x{{ptfankuan}}金币 = {{setAmount*parseFloat(ptfankuan)|tofixed}}</span> -->
                </p>
                <p>平台代确认截图：买手完成后系统将自动确认任务截图，减少商家审核时间（系统自动审核为批量审核，不负责查看任务截图）若商家自己手动审核截图等任务结束后结算退还，订单金额截图{{parseFloat(ptjietu)/2|tofixed}}金币/单，评价截图{{parseFloat(ptjietu)/2|tofixed}}金币/单</p>
                <p>本次待确认截图合计：<span  class='color-red'>{{setAmount+`单`}}x{{ptjietu}}金币 = {{setAmount*parseFloat(ptjietu) |tofixed}}</span></p>
              </div>
                ...
                <div class="addservice_content">
                  ...
                  <template v-if="shopType=='tb'||shopType=='tm'">
                    ...
                    <el-row>
                      ...
                      <el-col :span="4" class="input_margin_top">
                        <el-select v-model="valuezm.state" placeholder="无限制" @change="addServerList(valuezm,optionsZm,'芝麻积分限制')" :disabled="match.state">
                          <el-option label="无限制" value=""></el-option>
                          <el-option v-for="item in optionsZm" :key="item.id" :label="item.value+' '+item.price+'金币'" :value="item.id"></el-option>
                        </el-select>
                      </el-col>
                    </el-row>
                    ...
                    <el-row style="border-bottom: 0;padding-bottom: 0;">
                      <el-checkbox v-model="match.state" @click.native="chooselimit($event)" @change="addServerListRadio(match.state,'智能匹配人群',20)">智能匹配人群<span class="color-red">20</span>金币 选择此服务，系统会将您发布的商品根据性别、年龄、购买力、淘气值等多维度智能匹配精准人群</el-checkbox>
                    </el-row>
                  </template>
                </div>
              </div>
              ...
              <div class="otherservice">
                ...
                <template v-if="activeName!='second'">
                  ...
                  <el-row v-if="backtype == 2">
                    <el-col :span="24">
                      <span>任务保险</span>
                      <span class="font_size " style="margin-left:50px;">买手退款，平台赔付。全保2金币/单，半保1金币/单。</span>
                    </el-col>
                    <el-col :span="24">
                      <template v-for="(item,index) of taskbxArr">
                        <el-col :span="5.5" :key="item.id" style="margin-right:30px;">
                          <el-radio v-model="taskbxTime.state" :label="item.id" @click.native="addServerListRadio(taskbxTime,'任务保险',taskbxPriceArr[index])">
                            {{item.value}}
                          </el-radio>
                        </el-col>
                      </template>
                  ...
                  <el-row>
                    ...
                    <template v-for="(item,index) of enterArr">
                      ...
                      <el-col :span="5" :key="item.id" v-if="item.id!='67'||(risenumFlag&&activeName != 'fourth')" style="margin-left: -20px;">
                        ...
                        <el-radio v-model="enterTime.state" :label="item.id" @click.native="addServerListRadio(enterTime,'访客入店时间优化',item.price)">
                    ...
                    <!-- 自定义下单时间弹窗 -->
                    <setTimeDiaLog :data="{obj:setTimeRangeArr,customDialog}" @dialog="setTimeData" v-if="customDialog" class="setTime"></setTimeDiaLog>
                  </el-row>
                </template>
                <!-- 自动审核上线 -->
                <el-row>
                  ...
                  <el-col :span="20" class="font_size" style="line-height: 20px;">
                    <span>人工审核时间为9点-21点，21点后发布的任务需审核上线请选择此服务；任务付款之后由系统自动审核通过；（发布任务请核实好任务上线时间，避免选错上线时间或无法人工审核耽误您的商品计划）</span>
                  </el-col>
                </el-row>
                ...
                <el-row>
                  ...
                  <el-col :span="24" style="margin-top: 5px;">
                    <el-col :span="4">
                      <el-radio v-model="period.state" :label="9527" @click.native="addServerListRadio(period,'限制下单任务周期',0)">
                        20天 <span class="color-red">0.00</span>金币
                      </el-radio>
                    </el-col>
                    <el-col :span="4" v-for="item of periodArr" :key="item.id">
                      <el-radio v-model="period.state" :label="item.id" @click.native="addServerListRadio(period,'限制下单任务周期',item.price)">
                ...
                <template v-if="shopType == 'albb'||shopType == 'tb'||shopType == 'tm'">
                  ...
                  <el-row style="height: 170px; border-bottom: 0;">
                    ...
                    <el-col :span="24" style="height: 75px; margin-top: 5px;">
                      ...
                      <el-col :span="22">
                        <el-input type="textarea" :rows="3" placeholder="" v-model.trim="jptkl.state" @change="addServerListRadio(jptkl.state,'竞品狙击')"></el-input>
                      </el-col>
                    </el-col>
                    <el-col :span="24" style="margin-top: 15px; letter-spacing: 1px;" class="font_size">
                      <el-col :span="8">
                        <span>设置浏览人数({{jptklArr[0].price}}金币/单)</span>
                        <el-input-number v-model="browsenum" :min="0" :max="setAmount" label="" style="width: 35%;"></el-input-number>
                        人
                      </el-col>
                      <el-col :span="8">
                        <span>设置收藏商品人数({{jptklArr[1].price}}金币/单)</span>
                        <el-input-number v-model="collectnum" :min="0" :max="browsenum" label="" style="width: 35%;"></el-input-number>
                        人
                      </el-col>
                      <el-col :span="8" style="text-align: right">
                        <span>设置加购人数({{jptklArr[2].price}}金币/单)</span>
                        <el-input-number v-model="shoppingnum" :min="0" :max="browsenum" label="" style="width: 35%;"></el-input-number>
                        人
                      </el-col>
                    </el-col>
                  </el-row>
              ...
              <div class="otherservice font_size">
                ...
                <!-- <p v-for="item of evaluationArr" :key="item.id">
                  <el-radio v-model="evaluation.state" :label="item.id" @click.native="cancelRadio($event,'evaluation'),addServerListRadio(evaluation,'好评优化',item.price)">
                    {{item.id=='84'?'关键词好评：选择此服务后，系统将会以下关键词随即分配给买手，买手评价内容需涉及分配的相关关键词 (1金币/单)':''}}
                    {{item.id=='83'?'五星好评：选择此服务后，接手任务买手将商品5分默认好评 (0金币/单)':''}}
                  </el-radio>
                </p> -->
                <p>
                  <el-radio v-model="evaluation.state" :label="9527" @click.native="addServerListRadio(evaluation,'好评优化',0)">
                    默认好评：选择此服务后，接受任务买手将自由发挥15字以内的好评 (0金币/单)
                  </el-radio>
                </p>
                <p>
                  <el-radio v-model="evaluation.state" :label="83" @click.native="addServerListRadio(evaluation,'好评优化',0)">
                    五星好评：选择此服务后，接手任务买手将商品5分默认好评 (0金币/单)
                  </el-radio>
                </p>
              </div>
              ...
              <div class="otherservice">
                <!-- 关键词好评 -->
                <el-row style="border-bottom: 0; margin-bottom: 0;padding-bottom:10px">
                  <el-col :span="3">
                    <el-checkbox class='radio' v-model="evaluationGJC.state" @change="addServerListRadio(evaluationGJC.state,'关键词好评',evaluationGJC.price)">关键词好评</el-checkbox>
                  </el-col>
                  <el-col :span="20" class="font_size">
                    选择此服务后，系统将会以下关键词随即分配给买手，买手评价内容需涉及分配的相关关键词(<span class="color-red">{{evaluationGJC.price}}</span>金币/单)
                  </el-col>
                </el-row>
                <div class="keyword_box">
                  <div class="keyword_box_title">
                    请根据您发布任务的商品设定几个关键词作为访客的评价独自发挥撰写评价，例如”手感舒服、款式漂亮、包装讲究、物流很快、性价比高“等..请填写没有标点符号和空格的短词，关键字内容最多可填写10个字。不要填写完整的评价内容，避免所有访客评价内容一致
                  </div>
                  <div class="keyword_box_content clearfix">
                    <ul>
                      <li v-for="(i,index) in keywordlist" :key='index'>
                        关键字<span>{{index+1}}</span>：
                        <el-input v-model="keywordlist[index]" placeholder="请输入内容" style="width: 60%;" @change="GJCchange(keywordlist[index],index)"></el-input>
                      </li>
                    </ul>
                  </div>
                </div>
                <div class="set_keyword_box">
                  <div class="add_keyword">
                    ...
                    <el-button type="danger" @click="removeList('keywordlist','至少设置一组关键词')"><i class="fa fa-trash-o" aria-hidden="true"></i> 删除关键字</el-button>
                ...
                <el-row style="border-bottom: 0; margin-bottom: 0;padding-bottom:0">
                  <el-col :span="3">
                    <el-checkbox class='radio' v-model="msgtext.state" @change="addServerListRadio(msgtext.state,'自定义短语好评',1)">自定义短语好评</el-checkbox>
                  </el-col>
                  <el-col :span="20" class="font_size">
                    选择此服务后，选择此服务后系统将随机分配短语给买手(<span class="color-red">1</span>金币/组)
                  </el-col>
                </el-row>
                <el-row>
                ...
                <div class="set_imagetext_box">
                  <div class="add_imagetext">
                    <el-button type="success" @click="addList('msgList',setAmount*taskactionday)"><i class="fa fa-plus-square-o" aria-hidden="true"></i> 添加自定义好评</el-button>
                    <el-button type="danger" @click="removeList('msgList','至少设置一组自定义短语')"><i class="fa fa-trash-o" aria-hidden="true"></i> 删除自定义好评</el-button>
                    <span class='maxlist'>组数不得超过总单数</span>
                  </div>
                </div>
                <!-- 分割线 -->
                <div style='border-bottom: 1px solid #ccc;margin: 30px 0;'></div>
                ...
                <div v-if="shopType!='albb'">
                  <div id="jump10"></div>
                  <el-row style="border-bottom: 0; margin-bottom: 0;">
                    <el-col :span="3" style="width: 10%;">
                      <el-checkbox class='radio' v-model="imagetext.state" @change="addServerListRadio(imagetext.state,'图文好评',3)">图文好评</el-checkbox>
                    </el-col>
                    <el-col :span="20" class="font_size">
                      选择此服务后，接手任务买手评价时需同时上传商家提供的图片进行评价(<span class="color-red">3</span>金币/组)
                    </el-col>
                  </el-row>
                  <div v-for="(i,index) in imgArr" :key="index">
                    <el-row style="height: 130px; border-bottom: 0;">
                      ...
                      <el-col :span="2" class="image_margin" v-for="(j,Jindex) in i.imgList" :key="Jindex">
                        <input @mouseover="showdetele(index,Jindex)" @mouseout="outdetele(index,Jindex)" type="file" class="uploadInput" @change="uploadimg({name:'imgArr',x:index,y:Jindex}, 'tuwen'+index+Jindex, 'tuwenimg'+index+Jindex, false)" :id="'tuwen'+index+Jindex" >
                        <img :src="j?j:'/static/img/bg.png'" :id="'tuwenimg'+index+Jindex">
                        <i @mouseover="showdetele(index,Jindex)" class="el-icon-delete deleteimg" :icon="'deleteimg'+index+Jindex" @click="imgdelete('imgArr',index,Jindex)"></i>
                      </el-col>
                    </el-row>
                  ...
                  <div class="set_imagetext_box">
                    <div class="add_imagetext">
                      <el-button type="success" @click="addList('imgArr',setAmount*taskactionday)"><i class="fa fa-plus-square-o" aria-hidden="true"></i> 添加图文好评</el-button>
                      <el-button type="danger" @click="removeList('imgArr','至少设置一组图文好评')"><i class="fa fa-trash-o" aria-hidden="true"></i> 删除图文好评</el-button>
                      <span class='maxlist'>最多可添加5组图文好评(不可超过总单数)</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class='taskStep' style="font-size:16px;" v-if="kuaidiflag">
            ...
            <div style='padding:20px;padding-bottom:40px'>
              ...
              <el-row style="padding-top:20px" v-if="kuaidi=='1'">
                <el-radio v-model="kuaiditype" label="1" v-if="shopType != 'jd'&&shopType != 'pdd'">
                ...
                <el-radio v-model="kuaiditype" label="3">
          ...
          <div class="total_submit taskStep" style="margin-top: -30px;padding-bottom: 70px;">
            <el-button type="danger" @click.native="submit()" id="submit">提交任务信息</el-button>
          </div>
          <!-- 费用合计  合计显示到弹窗，暂时隐藏-->
          <div class='taskStep hide' v-if="false">
            <div class="taskStep_title">
              <h2>第<span>{{kuaidiflag?step+3:step+2}}</span>步: 费用合计<span style="color:#FFE9AE;font-size:14px" v-if="(shopType=='tm' || shopType=='tb' )&& isvip==false">(除人气和回访任务单笔任务大于50单及以上，每单减一元佣金费)</span></h2>
            </div>
              ...
              <div class="total_detail">
                <el-row type="flex" align="middle">
                  ...
                  <el-col :span="8">
                    ...
                    <p>运费押金：<span>{{baoyoustate == "1"?ptyunfei:0}}元×{{setAmount}}单</span></p>
                  </el-col>
                ...
                <el-row type="flex" align="middle">
                  ...
                  <el-col :span="12">
                    <template v-if='isvip == false'>
                      <p>刷单佣金：<span>{{yongjin1}}元×{{setAmount}}单{{(activeName!='third'&&activeName!='fourth'&&setAmount>=50&&this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'))?' - '+(setAmount)+'元':''}}</span></p>
                    </template>
                    <template v-else>
                      <p>刷单佣金：<span>{{yongjin1}}元×{{setAmount}}单</span>
                        <span v-if="setAmount>=10&&setAmount<30">{{(this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb'))?' - '+(setAmount)+'元':''}}</span>
                        <span v-if="setAmount>=30&&setAmount<50">{{(this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb'))?' - '+(setAmount*2)+'元':''}}</span>
                        <span v-if="setAmount>=50">{{(this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb'))?' - '+(setAmount*3)+'元':''}}</span>
                      </p>
                      <p v-if="setAmount<10&&this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb')" class="color-red">本次发布任务{{setAmount}}单，还差{{10-setAmount}}单可享受每单佣金立减1元的优惠</p>
                      <p v-if="setAmount>=10&&setAmount<30&&this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb')" class="color-red">本次发布任务{{setAmount}}单，还差{{30-setAmount}}单可享受每单佣金立减2元的优惠</p>
                      <p v-if="setAmount>=30&&setAmount<50&&this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb')" class="color-red">本次发布任务{{setAmount}}单，还差{{50-setAmount}}单可享受每单佣金立减3元的优惠</p>
                      <p v-if="setAmount>=50&&this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb')" class="color-red">您当前已享受每单减3元优惠</p>
                    </template>
                  ...
                  <el-col :span="4">{{yongjinSum}}</el-col>
                </el-row>
                <el-row type="flex" align="middle">
                  <el-col :span="8">增值服务</el-col>
                  <el-col :span="10">
                    <p>平台代返款：<span>{{ptfankuan}}金币×<span v-if="price*count<300">{{setAmount}}单</span><span v-else>{{price}}元 x {{setAmount}}单 × {{count}}件</span></span></p>
                    <p>平台代确认截图：<span>{{(isvip&&setAmount>=10&&backtype=='2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb'))?0:ptjietu}}金币×{{setAmount}}单</span></p>
                    <p v-for="(item,index) of serverArr" :key="index+5.1">
                      {{item.name}}：
                      <span>{{item.price|tofixed}}金币{{item.name=="花呗号下单"||item.name=="提升买号权重" ||item.name=="任务保险"||item.name=="淘气值限制"||item.name=="买号信誉限制"||item.name=="加赏佣金"||item.name=="好评优化"||item.name=="关键词好评"?'×'+setAmount+'单':''}}</span>
                      <span>{{item.name=="图文好评"?'×'+imgArr.length+'组':''}}</span>
                      <span>{{item.name=="自定义短语好评"?'×'+msgList.length+'组':''}}</span>
                    </p>
                  </el-col>
                  <el-col :span="6">{{serverSum}}</el-col>
                </el-row>
                <el-row type="flex" align="middle" style="border-bottom: 0;" v-if="kuaidiflag">
                  ...
                  <el-col :span="10">
                    <span>
                      ...
                      <span class="color-red">
                        {{activeName != 'second' && activeName != 'fourth'?taskactionday+'天，共':""}}
                      </span>
                      {{priceSum}}
                    </span>
                  </el-col>
    ...
    <el-dialog
      custom-class="gjzmianze"
      title="温馨提示"
      :visible.sync="lifanmianze"
      :show-close="false"
      :close-on-click-modal="false"
      width="46%">
      <p>您当前选择的是<span class='color-red'>立返任务</span>，买手做任务只提交订单截图，其他截图均不提供；</p>
      <p style='margin-top: 5px;'>买手下单后本金立返，佣金虽低，<span class='color-red'>如发生恶意退款情况商家自负，平台概不负责，建议慎选！！！</span></p>
      <div style="margin-top:60px;text-align: center;">
        <el-button type="danger" @click.native="lifanmianze=false" style="padding: 10px 40px;letter-spacing: 2px;">确定</el-button>
        <button @click="lifanmianze=false;backtype=2" class='backbtn'>返回发布货返任务</button>
      </div>
    </el-dialog>
    <!-- 立返任务价格超出自动转换货返弹窗 -->
    <el-dialog
      custom-class="gjzmianze"
      title="温馨提示"
      :visible.sync="ligohuo"
      :show-close="false"
      :close-on-click-modal="false"
      width="46%">
      <p>您当前选择的是<span class='color-red'>立返任务</span>，此类型任务不可发价值200以上的任务，系统将自动为您转换为货返任务；</p>
      <div style="margin-top:60px;text-align: center;">
        <el-button type="danger" @click.native="ligohuo=false" style="padding: 10px 40px;letter-spacing: 2px;">确定</el-button>
      </div>
    </el-dialog>
    <!-- 选择自己发快递时的温馨提示 -->
    <el-dialog
      custom-class="gjzmianze"
      title="温馨提示"
      :visible.sync="selfkuaidi"
      :show-close="false"
      :close-on-click-modal="false"
      width="46%">
      <p style='line-height: 25px;'><span class='color-red'>为了提升商家资金安全性</span>，使用平台快递的货返任务出现恶意退款平台全赔（平台快递可自定义重量 自动签收 安全性较高，如有需要可提供快递底单和证明）</p>
      <p style='line-height: 25px;'><span class='color-red'>如商家自己发实物</span>（如板砖 洗衣粉 矿泉水等）出现恶意退款，平台最多只赔付任务商品金额的50%，其他损失由商家自负，建议慎选</p>
      <div style="margin-top:60px;text-align: center;">
        <el-button type="danger" @click.native="selfkuaidi=false" style="padding: 10px 40px;letter-spacing: 2px;">确定</el-button>
        <button @click="selfkuaidi=false;kuaidi='1'" class='backbtn'>使用平台快递</button>
      </div>
    </el-dialog>
    <!-- 提交任务弹窗 -->
    <el-dialog
      custom-class="gjzmianze"
      title="费用详情"
      :visible.sync="submitPop"
      :show-close="false"
      :close-on-click-modal="false"
      width="46%">
      <div class='taskStep'>
        <div class="taskStep_content_total" style="padding:0">
          <el-row style="background: #ff908f; color: #fff; padding: 10px 90px; margin-bottom: 10px;">
            <el-col :span="8">分类</el-col>
            <el-col :span="8">费用明细</el-col>
            <el-col :span="8" style='text-align:right;'>合计</el-col>
          ...
          <div class="total_detail" style="padding: 0 40px;">
            <el-row type="flex" align="middle">
              <el-col :span="8">押金</el-col>
              <el-col :span="8">
                <p>商品押金：<span>{{price}}元 x {{setAmount}}单 × {{count}}件</span></p>
          ...
          <div class="total_cost">
            <el-row>
              ...
              <el-col :span="4" :offset="6">
                押金：<span class="color-red">{{Scommodityfrmoney}}</span>
              </el-col>
              <el-col :span="4">
                金币：<span class="color-red">{{Sgold}}</span>
              </el-col>
            </el-row>
          </div>
        </div>
      ...
      <div style="margin-top:40px;text-align: center;">
        <el-button type="danger" @click.native="submitPop=false;submit(1)" style="padding: 10px 40px;letter-spacing: 2px;">确定</el-button>
        <button @click="submitPop=false" class='backbtn'>取消</button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      lifanmianze: false,   // 立返任务免责条款
      ligohuo: false,   // 立返任务切换货返弹窗
      selfkuaidi: false,   // 立返任务切换货返弹窗
      editOrderno:'',     //
      editDatearea:'',
      ...
      shopTypeList:[       // 模拟接口获取平台类型数据
        {'id':1,val:'tb','name':'淘宝','reg':'(\.taobao\.|\.fliggy\.)'},
      ...
      renqiListAll: [false],   // 默认人气任务全选为false
      renqiList:[[]],          // 人气任务的数组
      ...
      yongjinArr:[],
      ptjietu: '',
      ptfankuan: '',   //  默认平台待返款金额，与ptfankuan1相同
      ptfankuan1: '',  //  用来接收价格小于300时的平台返款金额
      ptfankuan2: '',  //  用来接收价格大于等于300时的平台返款金额
      ptyunfei: '',
      isvip: false, //当前用户是否是VIP，默认false
      submitPop: false, // 提交弹窗
      Smoneysum: 0,         // 后台返回合计
      Scommodityfrmoney: 0, // 后台返回押金
      Sgold: 0,             // 后台返回金币
    }
  },
  ...
  async mounted () {
    ...
    //获取人气计划信息
    await this.getPlanData(1,1,this.backtype,0)
    //获取省市数据
    ...
    //处理默认的快递数据
    //log:1中通 2 圆通 3 申通 ；
    //pctype：1：淘宝2：天猫3：京东4：拼多多5：阿里巴巴 ,进入页面默认中通、淘宝
    let pctype = this.shopTypeList.find(e=>e.val == this.shopType)
    let item = this.getLog.find(e=>e.log == this.kuaiditype&&e.pctype == pctype.id)
    this.kuaidiPrice = item.price
  },
  components: {
    Header,
    RenqiPlan,
  ...
  methods:　{
    ...
    async getinfor(orderno,datearea){
      ...
      // 佣金计算需要 价格*件数    (一天单数//data1.ordertotal 取消不乘）
      let price = data1.commodityprice*data1.personbuycount
      await this.getPlanData(data1.pctype,data1.ordertype,data1.limitcashtype,price)
      ...
      if(data1.pctype == 4){
        this.pddCheckList = data1.buytype
      }
      //第二步数据
      ...
      data2.forEach((element,index)=>{
        ...
        if(index<data2.length-1){
          let jsonkey = this.$options.data().keywordArr[0]
          jsonkey.options = this.options
          this.keywordArr.push(jsonkey)

          this.renqiList.push(this.$options.data().renqiList[0])
           this.renqiListAll.push(this.$options.data().renqiListAll[0])
        }
        //开始获取数据
        ...
        if(data1.ordertype == 3){
          let arr = element.basedatacostids.split(',')
          this.renqiList[index] = arr.map(item=>parseInt(item))
          if(this.renqiList[index].length == this.renqiPlanArr.length){
            this.renqiListAll[index] = true
          }
          // 处理数据，不选的要为null
          this.renqiPlanArr.forEach((ele,i)=>{
            if(ele.id != this.renqiList[index][i]){
              //如果是最后一个元素就只需要填一个null即可
              if(i != this.renqiPlanArr.length-1){
                this.renqiList[index].splice(i,1,null,this.renqiList[index][i])
              }else{
                this.renqiList[index].splice(i,1,null)
              }
            }
          })
          //处理佣金计算
      ...
      data4.forEach(element=>{
        switch(element.classtype){
          ...
          case '19':
            ...
            //关键词好评优化
            if(element.addserviceid == 84){
              this.evaluationGJC.state = true
              let evaluationItem = this.evaluationArr.filter(item=>element.addserviceid == item.id?item:'')
              this.addServerListRadio(this.evaluationGJC,'关键词好评',evaluationItem[0].price)
          ...
          // 任务保险
          case '27':
            this.taskbxTime.state = element.addserviceid
            // let taskbxPrice = this.taskbxArr.filter(item=>element.addserviceid == item.id?item:'')
            // this.addServerListRadio(this.taskbxTime,'任务保险',this.taskbxPriceArr[0])
            // 选择全赔
            if(element.addserviceid == 110){
              this.addServerListRadio(this.taskbxTime,'任务保险',2)
            }
            // 选择半赔
            if(element.addserviceid == 111){
              this.addServerListRadio(this.taskbxTime,'任务保险',1)
            }
            // 不需要
            if(element.addserviceid == 109){
              this.addServerListRadio(this.taskbxTime,'任务保险',0)
            }
          break;
        }
      })
      ...
      let item = this.getLog.find(e=>e.log == data1.log&&e.pctype == data1.pctype)
      this.kuaidiPrice = item.price
    },
    // 获取页面数据信息
    async getBaseData(shopType){
      ...
      //   return false
      // }
      //2019-3 活动，非会员可以发任务，会员发任务有减免活动
      if(data.isvip == 2 && this.$route.fullPath == '/task'){
        this.isvip = true;
      }


      // 获取平台截图、返款、运费价格
      this.ptyunfei = data.cf.freightmoney
      this.ptjietu = data.cf.picbyplatform
      this.ptfankuan = data.cf.refundsbyplatform
      this.ptfankuan1 = data.cf.refundsbyplatform
      this.ptfankuan2 = data.cf.highrefundsbyplatform
      // 其他数据
      ...
      data.data.forEach(element=>{
        switch(element.classtype){
          ...
          case '11':
            ...
            // 设置默认增值服务
            this.addServerListRadio(this.enterTime,'访客入店时间优化',0)
          break;
          // 提升任务完成速度
          case '12':
          ...
          case '18':
            ...
            // 此处jptklArr 为保证顺序的话，可以修改为循环element.data进行赋值；
            let arr = new Array(3)
            element.data.forEach((element)=>{
              element.id == '80'?arr[0]=element:'' //浏览
              element.id == '81'?arr[1]=element:'' //收藏
              element.id == '82'?arr[2]=element:'' //加购
            })
            this.jptklArr = arr
            this.jptkl.serviceId = arr
          ...
          case '19':
            ...
            this.evaluationGJC.id = element.classtype
            //循环获取id 赋值给serviceid
            element.data.forEach(e=>{
              if(e.id == 84){
                this.evaluationGJC.serviceId = e.id
                this.evaluationGJC.price = e.price
              }
            })
            //默认是默认好评
            this.evaluation.state = 9527
          ...
          // 任务保险
          case '27':
            this.taskbxTime.id = element.classtype
            // 任务保险默认不需要
            this.taskbxTime.state = 109
            this.taskbxArr = element.data
            //设置默认增值服务
            this.addServerListRadio(this.taskbxTime,'任务保险',0)
          break;
        }
    ...
    async getPlanData(pctype,ordertype,limitcashtype,price){
      // 获取对应人气计划数据
      let data = await this.ajax({
        url: `PublishInterface/rewardrank?tk=${sessionStorage.token}`,
        data: {pctype,ordertype,limitcashtype,price},
        needAlert:false
      })
      if(!data) return false
      // 赋值首日到店价格
      ...
      // 如果是回访就触发佣金价格计算
      if(ordertype == 4){
        this.yongjinArr = this.$options.data().yongjinArr
        data.popu.forEach(element=>{
          this.yongjinArr.push({'title':element.title,'amount':this.setAmount,'radio':parseFloat(element.money),'price':parseFloat(element.money)*this.setAmount})
        })
      }
    },
    ...
    chooseitem (item,str) {
      ...
      if(item == 'shopType'){
        ...
        if(str=='jd'||str=='pdd'){
          ...
          if(str=='pdd'){
            this.$nextTick(()=>{
              let item = this.getLog.find(e=>e.log == '3'&&e.pctype == '4')
              this.kuaidiPrice = item.price
            })
          }else{
              this.$nextTick(()=>{
                //价格获取后台传过来的数据
                //log:1中通 2 圆通 3 申通 ；
                //pctype：1：淘宝2：天猫3：京东4：拼多多5：阿里巴巴
                let item = this.getLog.find(e=>e.log == '3'&&e.pctype == '3')
                this.kuaidiPrice = item.price
              })
          }
          // 京东拼多多没有花呗、竞品狙击
          this.serverArr = this.serverArr.filter(element=>{
            if(element.name != '花呗号下单' && element.name != '竞品狙击'){
              return element
            }else if(element.name == '花呗号下单'){
              this.huabei.state = ''
            }else if(element.name == '竞品狙击'){
              this.jptkl.state = ''
            }
          })
        }else{
        ...
        if(str=='albb'){
          this.serverArr = this.serverArr.filter(element=>{
            if(element.name != '图文好评'){
              return element
            }else{
              this.imagetext.state = false
              this.imgArr = this.$options.data().imgArr
            }
          })
        }
        //循环获取ID，重置人气计划数据
        this.shopTypeList.forEach(element=>{
          if(element.val == str){
            ...
            this.getPlanData(element.id,this.activeNameId,this.backtype,price)     //切换人气计划数据
          }
        })
        //切换时重置增值服务数据
        ...
        this.valuexy.state = ''
        this.valuezm.state = ''
        this.valuejingxiang.state = ''
        this.limitType1.state = ''
        this.limitType2.state = ''
        this.limitValue1 = ''
        this.limitValue2 = ''
        this.match.state = false
        this.serverArr = this.serverArr.filter(element=>{
          if(element.name != '性别限制'&&element.name != '年龄限制'&&element.name != '购买力限制'&&element.name != '淘气值限制'
            &&element.name != '买号信誉限制'&&element.name != '芝麻积分限制'&&element.name != '京享值限制'&&element.name != '会员等级限制'
            &&element.name != '消费标签限制'&&element.name != '智能匹配人群'){
            return element
          }
        })
      }
      ...
      if(item == 'backtype'){
        ...
        if(str == 1){
          // 如果立返任务超出500 需要切换货返
          if(this.price*this.count>200){
            this.ligohuo = true
            // this.backtype = 2
            this.$nextTick(()=>{
              this.chooseitem('backtype','2')
            })
          }else{
            this.lifanmianze = true
          }
          // 立返取消任务保险
          this.taskbxTime.state=109
          this.addServerListRadio(this.taskbxTime,'任务保险',0)
    ...
    async checkurl () {
      ...
      //如果是拼多多或者阿里巴巴则不进行判断
      if(this.shopType == 'albb' || this.shopType == 'pdd'){
        return false
      }
      if(flag){
        let data = await this.ajax({
    ...
    changePrice(val){
      this.shopTypeList.forEach(element=>{
        if(element.val == this.shopType){
          let price = val*this.count
          this.getPlanData(element.id,this.activeNameId,this.backtype,price)
        }
      })
      // 如果立返任务超出500 需要切换货返
      if(this.backtype == 1 && val*this.count>200){
        this.ligohuo = true
        // this.backtype = 2
        this.$nextTick(()=>{
          this.chooseitem('backtype','2')
      ...
      // 如果价格大于等于300 平台待返款金额为8/1000
      if(val*this.count>=300){
        this.ptfankuan = 0.008
      }else{
        this.ptfankuan = 2
      }
    },
    ...
    removekeyword (index) {
      if(this.keywordArr.length > 1){
        ...
        // 删除人气任务的数据
      if(this.activeName == 'third'){
        this.renqiList.splice(index, 1)
        this.renqiListAll.splice(index, 1)
      }
      }else{
        Message({
          message: `最少需设置1组关键字`,
    ...
    addList (item,length = 5) {
      ...
      if(this[item].length < length){
        ...
        let msg = length==0?`请先选择下单数量`:`最多只能设置${length}组自定义短语`
        if(item == 'keywordlist'){
          msg = `最多只能设置${length}组关键字`
        }
        if(item == 'imgArr'){
          msg = length==0?`请先选择下单数量`:`最多只能设置${length}组图文好评`
        }
    ...
    changeTabs (obj){
      ...
      if(typeof obj == 'object'){
        ...
        //如果选择凌晨单，需要把增值服务访客入店时间分布优化删除
        if(orderid == 2){
          this.serverArr = this.serverArr.filter(element=>{
            if(element.name != '访客入店时间优化'){
              return element
            }else{
              this.enterTime.state = this.enterArr[0].id
            }
          })
        }
        //显示关键词免责弹窗
      ...
      this.shopTypeList.forEach(element=>{
        if(element.val == this.shopType){
          let price = this.price*this.count
          this.getPlanData(element.id,orderid,this.backtype,price)     //切换人气计划数据
        }
      })
    ...
    addServerList (obj,arr,name){
      // 加赏佣金需要*单数，其他的默认1即可
      let ordernum = 1
      if(name=="淘气值限制" || name=="买号信誉限制"){ordernum = 0}
      //如果此服务被选中，则添加进增值服务；
      if(obj.state){
        arr.forEach(element=>{
    ...
    addServerListRadio (item,name,price){
      ...
      this.$nextTick(()=>{
        ...
        // 图文好评需要*组数 用2代表
        let ordernum = 1
        if(name=="加赏佣金" || name=="好评优化" || name=="关键词好评" || name=="提升买号权重" ||name=="任务保险" || name=="花呗号下单"){ordernum = 0}
        if(name=="图文好评" || name=="自定义短语好评"){ordernum = 2}
        ...
        if(item){
              ...
              this.serverArr.forEach(element=>{
                if(element.name == name){
                  // 处理好评优化价格一致 导致深浅复制BUG；
                  if(name == "好评优化"){
                    this.evaluation =  JSON.parse(JSON.stringify(obj))
                  }
                  // if(name == "任务保险"){
                    // this.taskbxTime =  JSON.parse(JSON.stringify(obj))
                  // }
                  element.price = price
                  flag = true
                }
        ...
        if(name=="关键词好评" && item){
          this.msgtext.state = false
          this.addServerListRadio(this.msgtext.state,'自定义短语好评',1)
          this.imagetext.state = false
          this.addServerListRadio(this.imagetext.state,'图文好评',1)
        }
        if(name=="自定义短语好评" && item){
          this.evaluationGJC.state = false
          this.addServerListRadio(this.evaluationGJC.state,'关键词好评',this.evaluationGJC.price)
          this.imagetext.state = false
          this.addServerListRadio(this.imagetext.state,'图文好评',1)
        }
        if(name=="图文好评" && item){
    ...
    setCustom () {
      ...
      if(this.enterTime.state=="67" && this.setAmount != 0){
        if(this.taskactionday>1){
          Message({
            message: `执行任务天数为一天才可设置自定义优化`,
            duration: 1500,
            type: 'error',
            showClose:'true'
          })
        ...
        //如果执行天数为0 就显示一条弹窗数据
        // if(this.taskactionday == 1){
        //   this.setTimeRangeArr = this.$options.data().setTimeRangeArr
        ...
        // }else{
          ...
          if(this.setTimeRangeArr.length != this.taskactionday){
            ...
            this.setTimeRangeArr = this.setTimeRangeArr.filter(item=>{
              item.amount = this.setAmount
              return item
            })
          }
        // }
    ...
    async saveAddress () {
      ...
      let hair_addr_detail = this.addresser.replace(/[\r\n]/g,"")
      ...
      // 需要验证的数据
      let validataArr = [
        {'val': province, 'type': 'isnull', 'name': '发货地址'},
        {'val': city, 'type': 'isnull', 'name': '发货地址'},
        {'val': hair_addr_detail, 'type': 'isnull', 'name': '详细地址'},
        {'val': addresser, 'type': 'isnull', 'name': '发货人姓名'},
        {'val': addresser_num, 'type': 'phone', 'name': '发货人电话'}
      ]
    ...
    chooselimit (event) {
      this.$nextTick(()=>{
        if(this.match.state && event.target.tagName == "INPUT"){
          // 选择智能匹配时进行增值服务的清除
          this.serverArr = this.serverArr.filter(element=>{
            let serverName = {
              "性别限制": 1,
    ...
    //处理人气计划的佣金，当前处理方式为全部数据情况重新计算-用时0.2ms,后期可以考虑优化成只处理用户选择的下标数据
    calcYongJin(){
      this.$nextTick(()=>{
        let arr = []
        // 用户选择的人气计划
        // 统一处理成json
        this.renqiList.forEach((element,i)=>{
          element.forEach((ele)=>{
            this.renqiPlanArr.forEach((e)=>{
              if(ele == e.id){
                let num = this.keywordArr[i].num
                arr.push({'title':e.title,'amount':num,'radio':parseFloat(e.money),'price':parseFloat(e.money)*num})
              }
            })
          })
        })
        // 处理数据至yongjinArr,先清空yongjinArr
        this.yongjinArr = this.$options.data().yongjinArr
        arr.forEach((element)=>{
          let flag = false
          this.yongjinArr.forEach((e,i)=>{
            if(element.title == e.title){
              flag = true
              let Nnum = element.amount+e.amount
              this.yongjinArr[i] = {'title':element.title,'amount':Nnum,'radio':parseFloat(element.radio),'price':parseFloat(element.radio)*Nnum}
            }
          })
          if(!flag){
            this.yongjinArr.push(element)
          }
        })
      })
    },
    // 处理人气任务全选
    chooseRenqiList (index,num) {
      const val = this.renqiListAll[index]
      if(val){
        this.renqiPlanArr.forEach((element,i)=>{
          this.$set(this.renqiList[index],i,element.id)
        })
        // 计算佣金
        this.calcYongJin()
      }else{
        // 如果renqiList里面没有null就说明现在是全部选择的状态，可以全部取消；
        if(this.renqiList[index].filter(item=>item==null).length==0){
          this.renqiList[index].map(item=>null).forEach((e,i)=>{
            this.$set(this.renqiList[index],i,null)
          })
          this.calcYongJin()
        }
      }
    ...
    // 处理人气任务选择
    chooseRenqi(index,index1){
      const val = this.renqiList[index][index1]
      // 处理全部的勾选状态
      if(this.renqiListAll[index] && !val){
        this.$set(this.renqiListAll,index,false)
      }
      if(!this.renqiListAll[index] && val){
        // 如果length与数据一致，并且当前没有null,就需要全选
        // let flag = this.renqiList[index].length == this.renqiPlanArr.length
        // if(this.renqiList[index].filter(item=>item==null).length==0 && flag){
        //   this.$set(this.renqiListAll,index,true)
        // }
        let flag = true
        for(let i=0;i<this.renqiPlanArr.length;i++){
          if(this.renqiList[index][i] == undefined){
            flag = false
          }
        }
        if(flag){
    ...
    //限制关键词长度十个字
    GJCchange(value,index){
      this.$nextTick(()=>{
        if(value.trim().length>10){
          // this.$set(this.keywordlist,index,'')
          Message({
            message: `关键词最多10个字`,
            duration: 1500,
            type: 'error'
    ...
    //鼠标移入显示删除图标
    showdetele(i,j){
      // let s= document.querySelector(`#tuwenimg${i}${j}`)
      // if(s.src.indexOf('/img/bg.png')==-1){
      //   let obj = document.querySelector(`.deleteimg[icon="deleteimg${i}${j}"]`)
    ...
    imgdelete(item,i,j){
      let s= document.querySelector(`#tuwenimg${i}${j}`)
      if(s.src.indexOf('/img/bg.png')==-1&&s.src.indexOf('data')==-1){
        // 清空数据
        this[item][i].imgList[j] = ""
        // 清空img图片
        document.querySelector(`#tuwenimg${i}${j}`).src="/static/img/bg.png"
        // 清空input文件
        document.querySelector(`#tuwen${i}${j}`).value = ''
      }
    },
    //提交发布任务信息
    async submit (isput = 0) {
      // isput 标识获取数据还是提交；0是获取价格数据，1是提交任务
      //状态码
      let flag = true
      ...
      if(this.shopType == 'pdd'){
        buytype = this.pddCheckList
      }
      // 拼多多需要验证必填支付方式
      if(this.shopType == 'pdd' && buytype == ''){
        Message({
          message: `请先选择支付方式`,
          duration: 1500,
          type: 'error'
      ...
      if(ordertype == 1||ordertype == 2||ordertype == 3||ordertype == 4||ordertype == 5||ordertype == 10){
        this.keywordArr.forEach((element,index)=>{
          //关键词、下单数、流量入口
          ...
          if(ordertype == 2){
            ...
            let validataArr = [
              ...
              {'val': endtime, 'type': 'isnull', 'name': '下单结束时间'}
            ]
            //如果验证不通过就return false
            if(!validata(validataArr)) {
              flag = false
          ...
          // 人气任务新增的人气计划数据
          let basedatacostids=[],basedatacosttext=[]
          if(ordertype == 3){
            this.renqiList[index].forEach(element=>{
              if(element!=null){
                this.renqiPlanArr.forEach(e=>{
                  if(e.id == element){
                    basedatacostids.push(e.id)
                    basedatacosttext.push(e.title)
                  }
                })
              }
            })
            // 人气任务必须设置进店方式
            if(basedatacostids.length==0){
              Message({
                message: `第${index+1}组关键词尚未设置人气计划`,
                duration: 1500,
                type: 'error'
              })
              flag = false
          ...
          basedatacostids = basedatacostids.join(',')
          basedatacosttext = basedatacosttext.join('+')
          data2.push({keyword,pordernum,keywordclass,type1,type2,type3,type4,
                    ...
                    priceend,sendarea,linchendate,startime,endtime,jobstate,basedatacostids,basedatacosttext})
        })
      }
      if(!flag) {
        this.jump("jump3")
      ...
      let firstdaynum=0
      // let addpersoncount=0  //暂时屏蔽，data1中不进行传参
        ...
        if(this.enterTime.state == '67'){
          ...
          this.setTimeRangeArr.forEach((element,i)=>{
            ...
            element.data.forEach(e=>{
              ...
              if(i == 0 && e.time != ""){
                let hour = e.time.slice(0,2)
                let min = e.time.slice(3,5)
                let sec = e.time.slice(6,8)
                let time = new Date(new Date(element.date).setHours(hour,min,sec)).getTime()
                if(time<Date.now()){
                  checktime = false
                }
              }
          ...
          if (sum != this.setAmount*this.taskactionday){
            Message({
      ...
      // 任务保险
      if(this.taskbxTime.state){
        let addserviceid = this.taskbxTime.id
        let othervalue = this.taskbxTime.state
        data4.push({addserviceid,othervalue})
      }
      // 提升完成速度
      ...
      // 限制下单任务周期 9527是默认20天，默认不需要传参数过去
      if(this.period.state  && this.period.state!=9527){
      ...
      // 好评优化 9527是默认好评，默认好评不需要传参数过去
      if(this.evaluation.state && this.evaluation.state!=9527){
      ...
      // 关键词好评
      if(this.evaluationGJC.state){
        // 先处理全部去空格
        this.keywordlist = this.keywordlist.map(e=>e.trim())
        for(var item of this.keywordlist){
          let index = this.keywordlist.indexOf(item)
          //判断字数
          if(item.length>10){
            Message({
              message: `第${index+1}个关键词超出字数限制`,
              duration: 1500,
              type: 'error'
            })
            this.jump('jump9')
        ...
        let addserviceid = this.evaluationGJC.id
        let othervalue = {val:this.keywordlist.join("@"),id:this.evaluationGJC.serviceId}
      ...
      if(this.msgtext.state){
        // 先处理全部去空格
        this.msgList = this.msgList.map(e=>e.trim())
        // 判断是否大于单数*天数
        if(this.msgList.length > (this.setAmount*this.taskactionday)){
            Message({
            message: `自定义短语组数设置超出限制`,
            duration: 1500,
            type: 'error',
            showClose:'true'
          })
        ...
        for(var item of this.msgList){
          let index = this.msgList.indexOf(item)
          // 判断value
          if(item == ""){
            Message({
              message: `第${index+1}组自定义短语未设置`,
              duration: 1500,
              type: 'error',
              showClose:'true'
            })
      ...
      if(this.imagetext.state){
        ...
        // 3-21新增判断自定义短语必填
        for(var i=0;i<othervalue.val.length;i++){
          // 自定义短语好评需要判断value
          if(othervalue.val[i].imgList.filter(item=>item != "").length == 0){
            Message({
              message: `第${i+1}组图文好评未设置图片`,
              duration: 1500,
              type: 'error',
              showClose:'true'
            })
          ...
          // 去除前后空格
          othervalue.val[i].text = othervalue.val[i].text.trim();
          // 如果上传了图片并且短语好评是空
          if(othervalue.val[i].imgList.filter(item=>item != "").length != 0&&othervalue.val[i].text ==""){
            Message({
              message: `第${i+1}组图文好评未设置短语`,
              duration: 1500,
              type: 'error',
              showClose:'true'
            })
        ...
        // 判断是否大于单数*天数
        if(this.imgArr.length > (this.setAmount*this.taskactionday)){
           Message({
            message: `图文好评组数设置超出限制`,
            duration: 1500,
            type: 'error',
            showClose:'true'
          })
      ...
      let moneysum = isput == 1?this.Smoneysum:this.priceSum //如果isput ==1 就调用后台传过来的数据；
      ...
      let data1 = {isput,id,datearea,pctype,shopid,clientype,commodityurl,commodityname,goodsdetail,commoditymasterimgurl,commoditypcimgurl,commodityprice,personbuycount,
                  ...
                  limitcredit,limitpoints,limitlabel,havingpic,isfirst,platformlog,logweight,log,logmoney,limitcashtype,ordertotal,ordertotalperson}
      ...
      if(isput == 0){
        this.submitPop = true
        this.Smoneysum = data.price.moneysum
        this.Scommodityfrmoney = data.price.commodityfrmoney
        this.Sgold = data.price.gold
      }else{
  ...
  computed: {
    ...
    yajinSum () {
      ...
      // 运费
      let b = (this.baoyoustate == "1"?parseFloat(this.ptyunfei):0)*this.setAmount
      return Math.ceil((a+b)*100)/100
    },
    // 佣金计算
    yongjin1 () {
      let yongjin1 = 0
      // let price = this.price*this.count*this.setAmount
      let price = this.price*this.count
      let flag = true
    ...
    yongjinSum () {
      ...
      if(!this.isvip&&(this.activeName!='third'&&this.activeName!='fourth'&&this.setAmount>=50&&this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'))){
        // 普通会员优惠
          a = this.yongjin1*this.setAmount-this.setAmount
      }else if(this.isvip&&(this.backtype == '2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb'))){
        //会员优惠
        if(this.setAmount>=10&&this.setAmount<30){
          a = this.yongjin1*this.setAmount-this.setAmount
        }else if(this.setAmount>=30&&this.setAmount<50){
          a = this.yongjin1*this.setAmount-(this.setAmount*2)
        }else if(this.setAmount>=50){
          a = this.yongjin1*this.setAmount-(this.setAmount*3)
        }else{
      ...
      return Math.ceil((a+sum)*100)/100
    },
    // 增值服务合计
    serverSum () {
      let sum = 0
      this.serverArr.forEach(element=>{
        ...
        if(element.ordernum == 0){
          sum += parseFloat(element.price)*parseInt(this.setAmount)
        }
        // 收一次费
        ...
        // 按组收费
        if(element.ordernum == 2){
          if(element.name == "图文好评"){
            sum += parseFloat(element.price)*parseInt(this.imgArr.length)
          }
          if(element.name == "自定义短语好评"){
            sum += parseFloat(element.price)*parseInt(this.msgList.length)
          }
        }
      })
      //计算平台待返款金额
      let ptkuan = 0
      if(this.price*this.count >= 300){
        ptkuan = this.price*this.setAmount*this.count*parseFloat(this.ptfankuan)
      }else{
        ptkuan = this.setAmount*parseFloat(this.ptfankuan)
      }
      //如果是VIP并且发布单数大于10
      if(this.isvip&&this.setAmount>=10&&this.backtype=='2'&&(this.shopType=='tb'||this.shopType=='tm'||this.shopType=='albb')){
        // 增值服务+平台待返款
        return Math.ceil((sum+ptkuan)*100)/100
      }else{
        // 增值服务+平台待返款+平台代确认截图
        return Math.ceil((sum+ptkuan+this.setAmount*parseFloat(this.ptjietu))*100)/100
      }
    },
    // 快递合计
    kuaidiSum () {
      if(this.shopType == 'albb'){
       return 0
      }else{
        return Math.ceil((this.kuaidiPrice*this.setAmount)*100)/100
      }
    },
    // 费用总计
    priceSum () {
      ...
      if(this.activeName != 'second' && this.activeName != 'fourth'){
        sum = (parseFloat(a+b+c+d)*this.taskactionday).toFixed(2)
      }
      //当达到要求时，需要sum+天数*单数，因为佣金里已经减免了，如果不加回去就会多减免钱；
      // if(this.activeName!='third'&&this.activeName!='fourth'&&this.setAmount>=50&&this.backtype == '2'&&(this.shopType!='jd'&&this.shopType!='pdd')){
      //   sum = (parseFloat(sum) + parseFloat(this.setAmount*(this.taskactionday-1))).toFixed(2)
      // }
      // 此处需要减去图文好评与自定义短语好评多余天数的钱
      let minus = 0
      this.serverArr.forEach(element=>{
        // 按组收费
        if(element.name == "图文好评"){
          minus += parseFloat(element.price)*parseInt(this.imgArr.length)
        }
        if(element.name == "自定义短语好评"){
          minus += parseFloat(element.price)*parseInt(this.msgList.length)
        }
      })

      return (sum-minus*(this.taskactionday-1)).toFixed(2)
    },
    // 押金费用总计
    yajinPriceSum(){
    ...
    //处理任务保险价格数据
    taskbxPriceArr(){
      // let price = this.price*this.setAmount*this.count
      // let quanpei = (price*0.01).toFixed(2)
      // let banpei = (price*0.005).toFixed(2)
      // 选择全赔
      // if(this.taskbxTime.state == 110){
        // this.addServerListRadio(this.taskbxTime,'任务保险',quanpei)
      // }
      // 选择半赔
      // if(this.taskbxTime.state == 111){
        // this.addServerListRadio(this.taskbxTime,'任务保险',banpei)
      // }
      // 价格等于0 时返的数组内价格不能一样，否则会不触发dom更新数据
      // if (price != 0){
      //   return [0,quanpei,banpei]
      // }else{
        return [0,2,1]
      // }
    },
  ...
  watch: {
    ...
    //切换快递类型
    kuaiditype (value) {
      //获取当前平台的ID 1：淘宝2：天猫3：京东4：拼多多5：阿里巴巴
      let pctype = this.shopTypeList.find(e=>e.val == this.shopType)
      ...
      if(this.kuaidi == "1"){
        if(value == "1") {
          ...
          //log:1中通 2 圆通 3 申通 ；
          //pctype：1：淘宝2：天猫3：京东4：拼多多5：阿里巴巴
          let item = this.getLog.find(e=>e.log == '1'&&e.pctype == pctype.id)
          this.kuaidiPrice = item.price
        ...
        if(value == "2") {
          ...
          //log:1中通 2 圆通 3 申通 ；
          //pctype：1：淘宝2：天猫3：京东4：拼多多5：阿里巴巴
          let item = this.getLog.find(e=>e.log == '2'&&e.pctype == pctype.id)
          this.kuaidiPrice = item.price
        ...
        if(value == "3") {
          ...
          //log:1中通 2 圆通 3 申通 ；
          //pctype：1：淘宝2：天猫3：京东4：拼多多5：阿里巴巴
          let item = this.getLog.find(e=>e.log == '3'&&e.pctype == pctype.id)
          this.kuaidiPrice = item.price
    ...
    kuaidi(value){
      if(value == "2"){
        ...
        this.selfkuaidi = true
      }else{
        this.kuaiditype = "0"
        this.$nextTick(()=>{
          this.kuaiditype = "1"
          if(this.shopType=='pdd'||this.shopType=='jd'){
            this.kuaiditype = "3"
          }
        })
      }
    ...
    count(a,b){
      this.$nextTick(()=>{
        ...
        // 如果立返任务超出500 需要切换货返
        if(this.backtype == 1 && a*this.price>200){
          this.ligohuo = true
          this.chooseitem('backtype','2')
          // this.backtype = 2
        }
        // 如果价格大于等于300 平台待返款金额为8/1000
        if(a*this.price>=300){
          this.ptfankuan = this.ptfankuan2
        }else{
          this.ptfankuan = this.ptfankuan1
        }
      })
    },
    // 总单数
    setAmount(a,b){
      this.$nextTick(()=>{
        ...
        // 如果是回访需要重置佣金价格中的单数
        if(this.activeName == 'fourth'){
          this.yongjinArr.forEach((element,index)=>{
            this.$set(element,'amount',this.setAmount)
            this.$set(element,'price',this.setAmount*element.radio)
            // this.yongjinArr[index].amount = this.setAmount
          })
        }
    ...
    collectnum(a,b){
      this.$nextTick(()=>{
        if(a+this.shoppingnum >this.browsenum){
          Message({
            message: `收藏人数加加购人数不可大于浏览人数`,
            duration: 1500,
            type: 'error',
            showClose:'true'
          })
    ...
    shoppingnum(a,b){
      this.$nextTick(()=>{
        if(a+this.collectnum >this.browsenum){
          Message({
            message: `收藏人数加加购人数不可大于浏览人数`,
            duration: 1500,
            type: 'error',
